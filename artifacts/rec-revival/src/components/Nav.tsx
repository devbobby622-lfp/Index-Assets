...
export default function Nav() {
  ...
  const showAdminBtn = currentUser?.email === 'darioncj112@gmail.com' || currentUser?.isAdmin;

  return (
    <>
      <nav
        className="fixed top-0 w-full z-40 backdrop-blur-md border-b border-border"
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {currentUser ? (
              <div className="relative" ref={menuRef}>
                ...
                {menuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-card border border-border rounded-2xl overflow-hidden z-50" style={{ boxShadow: '0 16px 48px rgba(0,0,0,0.35)' }}>
                    <Link href="/settings" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 text-sm font-bold hover:bg-muted transition-colors">
                      <Settings className="w-4 h-4 text-muted-foreground" /> Settings
                    </Link>
                    <button onClick={() => { signOut(); setMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold hover:bg-muted transition-colors text-red-400">
              ...
              <Link href="/sign-in" className="bg-card border border-border text-foreground px-4 py-2 rounded-full text-sm font-bold hover:border-primary/50 transition-colors">
                Sign In
              </Link>
            )}
            ...
            {showAdminBtn && (
              <button
                ...
                title="Admin Panel"
                className="flex items-center gap-1.5 bg-primary/15 border border-primary/30 text-primary px-3 py-2 rounded-full text-xs font-black hover:bg-primary/25 transition-colors"
